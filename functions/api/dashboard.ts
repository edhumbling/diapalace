...
export const onRequestGet: PagesFunction<CloudflareEnv> = async (context) => {
  try {
    ...
    const shiftScope = scoped("sc", scope);
    ...
    const shiftRow = await db.prepare(`SELECT COALESCE(SUM(sc.expected_cash), 0) AS expected, COALESCE(SUM(sc.counted_cash), 0) AS counted, COALESCE(SUM(sc.cash_difference), 0) AS variance FROM shift_closings sc WHERE sc.business_id = ? AND sc.closed_at >= ? AND sc.closed_at < ? AND ${shiftScope.sql}`).bind(authOrRes.user.business_id, window.start, window.end, ...shiftScope.params).first<{ expected: number; counted: number; variance: number }>();
    ...
    const varianceRows = await db.prepare(`SELECT sc.id, sc.cash_difference AS variance, sc.closed_at AS created_at, b.name AS branch_name FROM shift_closings sc LEFT JOIN branches b ON b.id = sc.branch_id WHERE sc.business_id = ? AND sc.cash_difference != 0 AND sc.closed_at >= ? AND sc.closed_at < ? AND ${shiftScope.sql} ORDER BY ABS(sc.cash_difference) DESC LIMIT 5`).bind(authOrRes.user.business_id, window.start, window.end, ...shiftScope.params).all<{ id: string; variance: number; created_at: string; branch_name: string | null }>();
    ...
    if (authOrRes.user.role === "owner" && !scope.branchId) {
      branches = await Promise.all(authOrRes.branches.map(async (branch) => {
        ...
        const shift = await db.prepare("SELECT COUNT(*) AS count FROM shift_closings WHERE business_id = ? AND branch_id = ? AND closed_at >= ? AND closed_at < ?").bind(authOrRes.user.business_id, branch.id, window.start, window.end).first<{ count: number }>();
