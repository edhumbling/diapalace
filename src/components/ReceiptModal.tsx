...
export default function ReceiptModal({ transaction, onClose }: ReceiptModalProps) {
  ...
  // Format items text for receipt printout/sharing
      ...
      (item) =>
        `${item.name}${
          item.variation
            ...
            : ""
        }\n  ${item.quantity} x GH₵ ${item.price.toFixed(2)} = GH₵ ${(
...
Payment:   ${transaction.paymentMethod}${
    transaction.momoNetwork ? ` (${transaction.momoNetwork})` : ""
  }
Status:    ${transaction.isVoided ? "VOIDED" : "PAID"}
================================
Thank you for your business!
Come back soon.
  `.trim();

  // Print helper: Opens a clean new window with receipt contents to print
  const handlePrint = () => {
    ...
    printWindow.document.write(`
      <html>
        ...
        <body>
          ...
          <p>================================</p>
          <p><strong>Payment:</strong> ${transaction.paymentMethod}${
      transaction.momoNetwork ? ` (${transaction.momoNetwork})` : ""
    }</p>
          <p><strong>Status:</strong> ${transaction.isVoided ? "VOIDED" : "PAID"}</p>
          <p>================================</p>
          <div class="center">
  ...
  const getSMSLink = () => {
    const encodedText = encodeURIComponent(fullReceiptText);
    const phoneNum = transaction.customer ? transaction.customer.phone.replace(/[^0-9+]/g, "") : "";
    // Note: SMS protocol differs slightly between iOS and Android, but body works universally
    return `sms:${phoneNum}?body=${encodedText}`;
  };
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="glass-panel w-full max-w-2xl rounded-3xl border border-zinc-800 p-6 flex flex-col md:flex-row gap-6 shadow-2xl animate-scale-up max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex-1 bg-white text-zinc-950 font-mono text-[10px] p-5 rounded-2xl shadow-inner max-h-[420px] overflow-y-auto border border-zinc-200">
          ...
          <div className="flex flex-col gap-2 mb-3">
            ...
            {transaction.items.map((item, idx) => {
              const displaySpecs = item.variation
                ? ` (${item.variation.size || ""}${
                    item.variation.size && item.variation.color ? "/" : ""
                  }${item.variation.color || ""})`
                : "";
              return (
                <div key={idx} className="flex justify-between items-start leading-tight">
                  <div className="pr-2">
                    ...
                    <span className="text-zinc-400 text-[8px] block">
          ...
          <div className="border-t border-dashed border-zinc-300 mt-4 pt-3 flex flex-col gap-0.5 text-zinc-500 text-[9px]">
            ...
            <div><strong>STATUS:</strong> {transaction.isVoided ? "VOIDED" : "PAID"}</div>
          </div>
        ...
        {/* Right Side: Receipt Sharing Options */}
        <div className="w-full md:w-60 flex flex-col justify-between gap-6">
          <div className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-2">
              ...
              <a
                href={getSMSLink()}
                className="w-full py-3 rounded-xl border border-sky-500/20 bg-sky-500/5 hover:bg-sky-500/10 text-sky-400 text-xs font-semibold flex items-center gap-3 px-4 transition-colors"
              >
